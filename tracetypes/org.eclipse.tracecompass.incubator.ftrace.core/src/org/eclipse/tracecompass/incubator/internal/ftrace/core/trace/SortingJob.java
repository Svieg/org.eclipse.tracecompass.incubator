...
final class SortingJob extends Job {
    ...
    protected IStatus run(IProgressMonitor monitor) {
        ...
        try (BufferedInputStream parser = new BufferedInputStream(new FileInputStream(fPath))) {
            ...
            String eventString = FtraceTrace.readNextEventString(() -> (char) parser.read());
            if (eventString == null) {
            ...
            while (eventString != null) {
                while (cnt < CHUNK_SIZE) {
                    ...
                    eventString = FtraceTrace.readNextEventString(() -> (char) parser.read());
                    if (eventString == null) {
                        break;
                    }
            ...
            for (File traceling : tracelings) {
                ...
                eventString = FtraceTrace.readNextEventString(() -> (char) createParser.read());
    ...
    private static @Nullable Pair readNextEvent(BufferedInputStream parser, int i) throws IOException {
        String event = FtraceTrace.readNextEventString(() -> (char) parser.read());
